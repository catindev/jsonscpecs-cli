...
function startStudio(project) {
  ...
  app.post('/v1/validate', (req, res) => {
    const body = req.body ?? {};
    if (!body.context || typeof body.context !== 'object') return res.status(400).json({ error: true, message: 'Request body must contain "context" object' });
    const pipelineId = body.context.pipelineId;
    if (!pipelineId || typeof pipelineId !== 'string') return res.status(400).json({ error: true, message: 'context.pipelineId is required (string)' });
    const payload = body.payload ?? {};
    const enrichedPayload = Object.assign({}, payload, { __context: body.context });
    try {
      const runtime = createCliEngine(project);
