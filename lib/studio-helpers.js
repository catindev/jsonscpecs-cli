  ...
  for (const [id, a] of compiled.registry) {
    if (a.type === 'pipeline' && a.entrypoint === true) {
      ...
      items.push({
        ...
        requiredContext: Array.isArray(a.required_context) ? a.required_context : []
      });
    }
  }
...
function analyzePipeline(rootPipelineId, compiled) {
  ...
  function walk(steps, depth, visited) {
    ...
    for (const step of steps) {
      stats.totalSteps++;
      if (step.kind === 'rule') {
        stats.rules++; stats.ruleIds.push(step.ruleId);
        if (step.ruleId?.startsWith('library.')) stats.librarySteps++; else stats.localSteps++;
      } else if (step.kind === 'condition') {
        stats.conditions++;
        if (step.conditionId?.startsWith('library.')) stats.librarySteps++; else stats.localSteps++;
        const cmp = compiled.conditions?.get(step.conditionId);
        if (cmp && !visited.has(step.conditionId)) { visited.add(step.conditionId); walk(cmp.steps, depth + 1, visited); }
      } else if (step.kind === 'pipeline') {
        stats.pipelines++;
        if (step.pipelineId?.startsWith('library.')) stats.librarySteps++; else stats.localSteps++;
        const cmp = compiled.pipelines?.get(step.pipelineId);
        if (cmp && !visited.has(step.pipelineId)) { visited.add(step.pipelineId); walk(cmp.steps, depth + 1, visited); }
      }
    }
  }
  ...
  const duplicates = Object.entries(ruleCount).filter(([, n]) => n > 1).sort((a, b) => b[1] - a[1]);
  if (stats.maxDepth >= DEPTH_WARN) stats.warnings.push(`Глубина вложенности ${stats.maxDepth}. Рекомендуется не более ${DEPTH_WARN - 1}`);
  if (stats.totalSteps >= STEPS_WARN) stats.warnings.push(`Всего шагов ${stats.totalSteps}. Сценарий может быть сложным для сопровождения`);
  const libPct = stats.totalSteps > 0 ? Math.round((stats.librarySteps / stats.totalSteps) * 100) : 0;
  if (libPct < LIBRARY_LOW && stats.totalSteps > 5) stats.warnings.push(`Только ${libPct}% шагов из библиотеки. Возможно стоит вынести правила в library`);
  for (const [id, n] of duplicates) stats.warnings.push(`Правило ${id} встречается ${n} раза в дереве. Возможен дубль`);
  return { totalSteps: stats.totalSteps, rules: stats.rules, conditions: stats.conditions, pipelines: stats.pipelines, maxDepth: stats.maxDepth, librarySteps: stats.librarySteps, localSteps: stats.localSteps, libraryPct: libPct, duplicates: duplicates.slice(0, 10), warnings: stats.warnings };
}
...
function buildTree(rootPipelineId, compiled, manifest) {
  ...
  function artifactLink(id) { return `/artifacts/${encodeURIComponent(id)}`; }
  function walkSteps(steps, out) {
    for (const step of steps || []) {
      if (step.kind === 'rule') {
        const art = compiled.registry.get(step.ruleId);
        out.push({ kind: 'rule', id: step.ruleId, title: titleForArtifact(step.ruleId), subtitle: art?.operator ? `${art.operator}${art.field ? ' · ' + art.field : ''}` : '', link: artifactLink(step.ruleId), children: [] });
      } else if (step.kind === 'condition') {
        const art = compiled.registry.get(step.conditionId);
        const cmp = compiled.conditions?.get(step.conditionId);
        const node = { kind: 'condition', id: step.conditionId, title: titleForArtifact(step.conditionId), subtitle: art?.description || '', link: artifactLink(step.conditionId), children: [] };
        out.push(node);
        if (cmp && !visited.has(step.conditionId)) { visited.add(step.conditionId); walkSteps(cmp.steps, node.children); }
      } else if (step.kind === 'pipeline') {
        const art = compiled.registry.get(step.pipelineId);
        const cmp = compiled.pipelines?.get(step.pipelineId);
        const node = { kind: 'pipeline', id: step.pipelineId, title: titleForArtifact(step.pipelineId), subtitle: art?.strict ? 'strict' : '', link: `/pipelines/${encodeURIComponent(step.pipelineId)}`, children: [] };
        out.push(node);
        if (cmp && !visited.has(step.pipelineId)) { visited.add(step.pipelineId); walkSteps(cmp.steps, node.children); }
      }
    }
  }
  const root = compiled.pipelines?.get(rootPipelineId);
...
function generatePipelineDoc(rootPipelineId, compiled, manifest, fmt = 'md') {
  ...
  function mono(s) { return fmt === 'wiki' ? `{{${s}}}` : `\`${s}\``; }
  function bold(s) { return fmt === 'wiki' ? `*${s}*` : `**${s}**`; }
  function h(level, t) { return fmt === 'wiki' ? `h${level}. ${t}` : `${'#'.repeat(level)} ${t}`; }
  function hr() { return fmt === 'wiki' ? '----' : '---'; }
  function fieldLabel(id) { return fields[id]?.description || id; }
  function walkPipeline(id, depth, visited) {
    ...
    for (const step of cmp?.steps || []) {
      if (step.kind === 'rule') {
        ...
        lines.push(`- ${bold(rule.description || step.ruleId)} (${mono(step.ruleId)})`);
        if (rule.field) lines.push(`  - ${bold('Поле')}: ${fieldLabel(rule.field)} (${mono(rule.field)})`);
        if (rule.operator) lines.push(`  - ${bold('Оператор')}: ${operators[rule.operator]?.description || rule.operator}`);
        if (rule.message) lines.push(`  - ${bold('Сообщение')}: ${rule.message}`);
      } else if (step.kind === 'condition') {
        const cond = compiled.registry.get(step.conditionId);
        lines.push(`- ${bold(cond.description || step.conditionId)} (${mono(step.conditionId)})`);
        ...
        if (ccmp?.steps?.length && !visited.has(step.conditionId)) {
          ...
          for (const nested of ccmp.steps) {
            if (nested.kind === 'rule') {
              ...
              lines.push(`  - ${bold(rule.description || nested.ruleId)} (${mono(nested.ruleId)})`);
            }
          }
        }
      } else if (step.kind === 'pipeline') {
...
function generateArtifactDoc(id, compiled, manifest, fmt = 'md') {
  ...
  if (art.field) lines.push(`${bold('Поле')}: ${field?.description || art.field} (${mono(art.field)})`);
  if (art.operator) lines.push(`${bold('Оператор')}: ${manifest.operators?.[art.operator]?.description || art.operator}`);
...
function analysisSummary(compiled, project, operatorMeta) {
  let ruleCount = 0, conditionCount = 0, pipelineCount = 0, dictionaryCount = 0, entrypointCount = 0;
  for (const [, art] of compiled.registry) {
    if (art.type === 'rule') ruleCount++;
    else if (art.type === 'condition') conditionCount++;
    else if (art.type === 'pipeline') { pipelineCount++; if (art.entrypoint === true) entrypointCount++; }
  ...
  return {
    ...
    dictionaryCount,
    entrypointCount,
    operatorPacks: Array.isArray(project.manifest.operatorPacks?.node) ? project.manifest.operatorPacks.node : [],
  ...
  for (const step of stepsInput || []) {
    if (step.kind === 'rule') {
      ...
      steps.push({
        ...
        steps: cyc ? [] : buildFlowModel(step.pipelineId, compiled, manifest, nextVisited)
      });
    } else if (step.kind === 'condition') {
      const art = compiled.registry.get(step.conditionId);
      const ccmp = compiled.conditions?.get(step.conditionId);
      ...
      const whenHtml = ccmp?.when
        ? require('./when-render').renderWhenTreeHtml(
            ...
            (predId) => {
              ...
              return '<a href="/rules/' + encodeURIComponent(predId) + '" class="flow-cond-rule-link">' + require('./when-render').escapeHtml(lbl) + '</a>';
            },
      ...
      steps.push({
        ...
        whenText: ccmp?.when ? require('./when-render').renderWhenText(ccmp.when, (predId) => {
          const pred = compiled.registry.get(predId); return (pred?.description || predId);
        }, { stripLeadingIf: true }) : '',
        whenHtml,
        steps: ccmp?.steps ? buildFlowSteps(ccmp.steps, compiled, manifest, nextVisited) : []
      });
    }
  }
  return steps;
...
function enrichArtifactForUi(id, compiled, manifest) {
  ...
  const display = {
    ...
    operator: artifact.operator ? manifest.operators?.[artifact.operator] || null : null,
  ...
  if (artifact.type === 'condition') {
    ...
    return {
      ...
      compiled: compiledCondition ? {
        ...
        whenText: compiledCondition.when ? require('./when-render').renderWhenText(compiledCondition.when, (predId) => {
          const pred = compiled.registry.get(predId); return pred?.description || predId;
        }, { stripLeadingIf: true }) : '',
        whenHtml: compiledCondition.when ? require('./when-render').renderWhenTreeHtml(
        ...
        steps: buildFlowSteps(compiledCondition.steps || [], compiled, manifest, new Set([id]))
      } : null,
      display
    };
  }
  const compiledArtifact = artifact.type === 'pipeline' ? compiled.pipelines?.get(id) : null;
